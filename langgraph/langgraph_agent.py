...
async def wait_for_stable_page(page: Page, selector: str, timeout: int = 10000) -> None:
...
@tool
async def navigate_to(url: str, page: Page) -> str:
    """Navigate to a specific URL."""
    await page.goto(url, wait_until="load")
    return f"Navigated to {url}"

@tool
async def click_text(text: str, page: Page) -> str:
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        # Bind page to tools (LangChain v1 pattern)
        for t in tools:
            if hasattr(t, "func"):
                original = t.func
                t.func = lambda *args, **kwargs: original(*args, page=page, **kwargs)
...
Always use the available tools. Prefer clicking by visible text for navigation.""",
        ...
        task = f"""
        1. Ensure we are logged in (username: {LOGIN_USERNAME}, password: {LOGIN_PASSWORD}). If not, log in.
        2. Open the hamburger menu in the top left.
        3. Click on "My Saved Lists".
        4. Take a screenshot of the resulting page.
        5. Describe what you see and note any useful patterns (e.g. how the nav works) for future runs.
        """
        ...
        final_knowledge = {
            "learned_patterns": knowledge.get("learned_patterns", []) + [result],
